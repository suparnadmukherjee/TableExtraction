...
from IDP_compressed.Unbordered.Xerox_Table.table_utils.row_detection import row_detection
...
def recognize_structure(img):
    ...
    thresh, img_bin = cv2.threshold(GryImg, 128, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    img_bin_inv = 255 - img_bin

    kernel_len_ver = max(10, img_height // 100)
    kernel_len_hor = max(10, img_width // 100)
    ...
    vh_subracted = GryImg * (1 - (vh_lines / 255).astype('uint8')) + vh_lines
    print("y1")
    ...
    ver_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, img_height // 5))  # shape (kernel_len, 1) inverted! xD
    vertical_lines = cv2.erode(img_median, ver_kernel, iterations=4)

    horizontal_lines, sep_line_mask = row_detection(vh_subracted)
    print("y2")
    # Combine horizontal and vertical lines in a new third image, with both having same weight.
    img_vh = cv2.addWeighted(vertical_lines, 0.5, horizontal_lines, 0.5, 0.0)
    # plt.imshow(img_vh)
    # plt.show()
    img_vh = cv2.bitwise_or(img_vh, vh_lines)
    ...
    cell_marked_image = img
    for c in contours:
        x, y, w, h = cv2.boundingRect(c)
        if (w < 0.9 * img_width and h < 0.9 * img_height):
            cell_marked_image = cv2.rectangle(img, (x, y), (x + w, y + h), (0, 0, 255), 2)
            box.append([x, y, w, h])
    print("y4")
    ...
    print("y6")
    return finalboxes, img_bin, cell_marked_image