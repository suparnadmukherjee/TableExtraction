    ...
    def process_single_page(self,document_img,file_path,coordinates,flag):
        ...
        for tab_counter,table in enumerate(table_list):
            #try:
                ...
                cv2.imwrite('IDP_compressed/Unbordered/Xerox_Table/output/Table_Cell_Marked/' + filename + '_tab_' + str(tab_counter+1) + '.jpg', table_cell_marked)
                ...
                with open('data/csv/idp1/'+str(filename) + str(tab_counter+1) + '.json', 'w') as f:
                    json.dump(parsed, f)
                result = output_to_xlsx(arr.T)
                result.to_excel('data/csv/idp1/' + str(filename)  + str(tab_counter+1) + '.xlsx')
                logging.info(f"'data/csv/idp1/{str(filename)}{str(tab_counter + 1)}.xlsx saved")
                # tab_counter += 1
